...
from skrl.trainers.torch import TrainerMPC
...
def fn_processor(process_index, *args):
    ...
    while True:
        ...
        # get agent's actions
            ...
            _mpc_states = queue.get()[scope[0] : scope[1]]
            with torch.no_grad():
                stochastic_evaluation = msg["stochastic_evaluation"]
                _outputs = agent.act(_states, _mpc_states, timestep=msg["timestep"], timesteps=msg["timesteps"])
...
class ParallelTrainerMPC(TrainerMPC):
    def __init__(
        self,
        env: Wrapper,
