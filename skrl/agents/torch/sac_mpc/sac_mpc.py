...
class SAC_MPC(Agent):
    def __init__(
        ...
        """
        _cfg = copy.deepcopy(SAC_MPC_DEFAULT_CONFIG)
        ...
        for gradient_step in range(self._gradient_steps):
            ...
            with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                ...
                with torch.no_grad():
                    next_actions, next_log_prob, _ = self.policy.act(
                        {"states": sampled_next_states, "mpc_states": sampled_mpc_states}, role="policy"
                    )
            ...
            with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                ...
                actions, log_prob, _ = self.policy.act(
                    {"states": sampled_states, "mpc_states": sampled_mpc_states}, role="policy"
                )
                critic_1_values, _, _ = self.critic_1.act(
